...
def surfDiceFun_1Class(val_labels, postImg_monai):
    ...
    for i in range(ypred_BCHW.shape[0] - 4):
        if np.sum(ylabe_BCHW[i:i + 2, 1, :, :]) > 0:  # or np.sum(ypred_BCHW[i:i+2,0,:,:])>0:
            hausdorff_metric(y_pred=ypred_BCHW[i:i + 2, :, :, :], y=ylabe_BCHW[i:i + 2, 0:, :, :])
            hausdorff_list.append(hausdorff_metric.aggregate().item())
            # print(hausdorff_list[-1])
            hausdorff_metric.reset()
    
    HausdorffMean = np.mean(hausdorff_list)
    
    return SurfDiceMean,HausdorffMean
...
def test(figures_folder_i,image_size,model, val_loader,device,val_files):
    ...
    with torch.no_grad():
        for val_data in val_loader:
            ...
            #SURFDICE
            sdice0,hausd0 = surfDiceFun_1Class(val_labels, val_outputs)
            sdice1,hausd1 = surfDiceFun_1Class(val_labels, val_outPost)
            print('SurfFice vanilla 1 class:', sdice0,' HausD: ',hausd0)
            print('SurfFice post 1 class:', sdice1,'HAusD: ', hausd1)

            #ALL TOGETHER
            all_metrics.append([val_files[count]['label'].split('/')[-2], dice0, dice1, sdice0, sdice1,hausd0,hausd1])
            count += 1
...
def main(figures_folder_i,name_run,pretrained_path,cache,num_workers,root_path):
    ...
    #Save csv of metrics
    if True:
        f= open(figures_folder_i+'res_UMCGx4_SwinUnet.csv','w', encoding='UTF8')
        writer = csv.writer(f)
        writer.writerow(['Patient','Dice V','Dice P','Sdice V','SDice P','HausD V','HausD P'])
...
if __name__ == "__main__":
    ...
    figures_folder_i = '/data/p308104/Figures/figures_SU_l3_v9/'
    pretrained_path = root_path+'best_SwinUnet_V9_NBIA_Loss3.pth'

    cache = False
    lf_select = None # NOT Needed for testing
    print('LF is ', lf_select, 'Db is UMCG', 'SwinUnet')
    name_run = "SwinUnet" + "LF" + str(lf_select) + "run1"
