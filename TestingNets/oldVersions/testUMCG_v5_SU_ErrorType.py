        ...
        for val_data in val_loader:
            ...
            if False:#Full volume
                for i in range(val_labels[0].shape[3]):
                    if np.sum(val_labels[0].detach().cpu()[0, :, :,i],)>0:
                        ...
                        plt.subplot(1, 1, 1),  plt.imshow(val_inputs.cpu().numpy()[0, 0, :, :, i]+ val_outputs[0].detach().cpu()[0, :, :,i],
                        ...
                        plt.savefig(os.path.join(figures_folder_i, px, 'FullV_final_V5{}.png'.format(i)))

            if figs:  # AXIAL
                slice_indices = []
                for i in range(val_inputs.shape[4]):
                    if np.sum(val_labels[0][0, :, :, i]) > 0:
                ...
                for i, idx in enumerate(slice_indices):
                    ...
                    plt.subplot(rows, nr_images, j), plt.title('AXIAL CT ({})'.format(idx)), plt.imshow(
                        val_inputs.cpu().numpy()[instance, 0, :, :, idx], cmap='gray', vmin=0, vmax=1), plt.axis('off')
                    plt.subplot(rows, nr_images, nr_images + j), plt.title('Prediction ({})'.format(idx)), plt.imshow(
                        val_inputs.cpu().numpy()[instance, 0, :, :, idx] + val_outputs[0].detach().cpu()[instance, :, :,idx]), plt.axis('off')
                    #plt.subplot(rows, nr_images, 2 * nr_images + j), plt.title(
                    #    'postImg Monai ({})'.format(idx)), plt.imshow(
                    #    val_inputs.cpu().numpy()[instance, 0, :, :, idx] + val_outPost[0].detach().cpu()[instance, :, :, idx]), plt.axis('off')
                    plt.subplot(rows, nr_images, 2 * nr_images + j), plt.title(
                        'Label ({})'.format(idx)), plt.imshow(
                        val_labels[0].detach().cpu()[instance, :, :, idx]), plt.axis('off')
                plt.tight_layout(), plt.show()
                if not os.path.exists(os.path.join(figures_folder_i, px)):
                    os.makedirs(os.path.join(figures_folder_i, px))

                plt.savefig(os.path.join(figures_folder_i, px, bp + 'AXIAL final_V5{}.png'.format(px)))
            if False:#figs:  # SAGITAL
                slice_indices = []
                ...
                for i, idx in enumerate(slice_indices):
                    j = 1 + i
                    fig = plt.figure('Instance = {}'.format(instance), figsize=figsize)
                    plt.subplot(rows, nr_images, j), plt.title('SAGITAL CT ({})'.format(idx)), plt.imshow(
                        val_inputs.cpu().numpy()[instance, 0, :, idx, :], cmap='gray', vmin=0, vmax=1), plt.axis('off')
                    plt.subplot(rows, nr_images, nr_images + j), plt.title('Prediction ({})'.format(idx)), plt.imshow(
                    ...
                    plt.subplot(rows, nr_images, 2 * nr_images + j), plt.title(
                        ...
                        val_inputs.cpu().numpy()[instance, 0, :, idx, :] + val_outPost[0].detach().cpu()[instance, :,
                                                                           idx, :]), plt.axis('off')
                    plt.subplot(rows, nr_images, 3 * nr_images + j), plt.title(
                        'Label ({})'.format(idx)), plt.imshow(
                        val_labels[0].detach().cpu()[instance, :, :, idx]), plt.axis('off')
                plt.tight_layout(), plt.show()
                if not os.path.exists(os.path.join(figures_folder_i, px)):
            ...
            if False:#figs:  # CORONAL
                ...
                for i, idx in enumerate(slice_indices):
                    j = 1 + i
                    fig = plt.figure('Instance = {}'.format(instance), figsize=figsize)
                    plt.subplot(rows, nr_images, j), plt.title('CORONAL CT ({})'.format(idx)), plt.imshow(
                        val_inputs.cpu().numpy()[instance, 0, idx, :, :], cmap='gray', vmin=0, vmax=1), plt.axis('off')
                    plt.subplot(rows, nr_images, nr_images + j), plt.title('Prediction ({})'.format(idx)), plt.imshow(
            ...
            if metrics:
                ...
                dif = 0#sum_labels - sum_outputs
                print('difference is: ',dif)

                #DICE
                #dice_metric(y_pred=val_outputs, y=val_labels)
                dice0 = 0 #dice_metric.aggregate().item()
                #print('Scores vanilla:', dice0)
                #dice_metric.reset()

                #dice_metric(y_pred=val_outPost, y=val_labels) #[0][1:2, :, :, :] [0]
                dice1 = 0#dice_metric.aggregate().item()
                ...
                hausd0 = hausdorff_metric.aggregate().item()
                hausdorff_metric.reset()
                hausdorff_metric(y_pred=val_outPost, y=val_labels)
                hausd1 = hausdorff_metric.aggregate().item()
                hausdorff_metric.reset()
                print('Haus vanilla 1 class:', hausd0)
                print('Haus post 1 class:', hausd1)

                #SURFDICE
                sdice0 = 0#surfDiceFun_1Class(val_labels, val_outputs)
                sdice1 = 0#surfDiceFun_1Class(val_labels, val_outPost)
                #print('SurfFice vanilla 1 class:', sdice0)
                #print('SurfFice post 1 class:', sdice1)


                #ALL TOGETHER
                all_metrics.append([val_files[count]['label'].split('/')[-2], dice0, dice1, sdice0, sdice1,hausd0,hausd1,dif])
            count += 1
...
if __name__ == "__main__":
    ...
    figures_folder_i = figures_path+'figures_DynU_l3_v9/'
    pretrained_path = preweight_path + 'best_DynUnet_V9_UMCG_Loss3.pth'
    name_run = "NameRun" + str(SelectModel) + "LF" + str(lf_select) + "run1"
    print(name_run)
    main(SelectModel, figures_folder_i, name_run, pretrained_path, cache, num_workers, root_path)
