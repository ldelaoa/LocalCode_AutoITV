...
def LookSortFiles(root_path, all_patientdir):
    ...
    for patient_path in all_patientdir:
        ...
        for root, dirs, files in os.walk(root_path + patient_path, topdown=False):
            for f in files:
                if "_gtv" in f.lower() and not("pred" in f.lower()):
                    gtv_fpaths.append(os.path.join(root_path, patient_path, f))
                    gtv_miss +=1
                if ("_igtv" in f.lower() or "itv" in f.lower()) and not ("pred" in f.lower()):
...
def EvalNet_fun(val_loader, modelSwin, modelDyn,image_size):
    ...
    return predicted_ITV,predicted_ITV_nopost,val_data,px,val_inputs
...
def evaluationPerPx(all_patientdir, root_path, pretrained_path_Swin, pretrained_path_Dyn, figures_folder_i,name_run):
    ...
    predicted_ITV,predicted_ITV_nopost, val_data, px, val_inputs = EvalNet_fun(val_loader, modelSwin, modelDyn, image_size)
    if True:
        print("Len of bp, YesPost and NoPost: ",len(predicted_ITV),len(predicted_ITV_nopost))
        for j in range(len(predicted_ITV)):
            print("File number: ",j,)
            path_to_save = os.path.join(root_path, px)
            saveNifti(predicted_ITV[j], path_to_save, '/GTV_YesPost_'+str(j)+'BP.Nii')
            saveNifti(predicted_ITV_nopost[j].detach().cpu().numpy(), path_to_save, '/GTV_NoPost_'+str(j)+'BP.Nii')


    if False:
        # interpolate ITV Tensor
        ITV_tensor, ITV_tensor_2BP, predicted_ITV_interp = rescaleITV(predicted_ITV)
        ITV_tensor_10BP_noPost,ITV_tensor_2BPnoPost,predicted_ITV_interp_nopost = rescaleITV(predicted_ITV_nopost)
    ...
    if False:
        path_to_save = os.path.join(root_path, px)
        saveNifti(ITV_tensor, path_to_save, '/predicteditv_allbp_yes.Nii')
        saveNifti(ITV_tensor_10BP_noPost.detach().cpu().numpy(), path_to_save, '/predicteditv_allbp_no.Nii')
    if False:
...
def main(root_path,pretrained_path_Swin,pretrained_path_Dyn,figures_folder_i,name_run):
    ...
    _ = saveMetrics(figures_folder_i, 'AllBP_metrics_v12.csv', metrics_allBP)
    _ = saveMetrics(figures_folder_i, 'TwoBP_metrics_v12.csv', metrics_twoBP)
