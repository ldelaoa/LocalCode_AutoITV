...
def test(figures_folder_i,image_size,modelSwin,modelDyn, val_loader,device,val_files):
...
def main(spec_patient,SelectModel,figures_folder_i,name_run,pretrained_path_Swin,pretrained_path_Dyn,cache,num_workers,root_path):
    ...
    all_metrics = test(figures_folder_i ,image_size,modelSwin,modelDyn, val_loader,device,val_files)


    #Save csv of metrics
    if True:
        f= open(figures_folder_i+str(SelectModel)+'res_UMCGx10_SwinDyn.csv','w', encoding='UTF8')
        writer = csv.writer(f)
