        ...
        for val_data in val_loader:
            ...
            for i in range(len(props)):
                ...
                if metrics:
                    ...
                    if val_outPost[0].shape[1]==288:
                        surfDice_metric287(y_pred=val_outPost[0], y=val_labels[0])
                        sdice1 = surfDice_metric287.aggregate().item()
                        surfDice_metric287.reset()
                    elif val_outPost[0].shape[1]==192:
                        surfDice_metric191(y_pred=val_outPost[0], y=val_labels[0])
                        sdice1 = surfDice_metric191.aggregate().item()
                        surfDice_metric191.reset()
                    else:
                        surfDice_metric383(y_pred=val_outPost[0], y=val_labels[0])
...
def main(spec_patient,SelectModel,figures_folder_i,name_run,pretrained_path_Swin,pretrained_path_Dyn,cache,num_workers,root_path):
    ...
    #Save csv of metrics
    if True:
        f= open(figures_folder_i+str(SelectModel)+'res_UMCGx7_ValidationSET_Accuracy.csv','w', encoding='UTF8')
        writer = csv.writer(f)
...
if __name__ == "__main__":
    ...
    figures_folder_i = figures_path + 'figures_SwinDyn_v10/'
